...
import Logo from "@/components/feed/shared/Logo";
import Image from "next/image";
import Link from "next/link";
import { useEffect, useRef, useState } from "react";

export default function Header() {
  const [isNotifyDropOpen, setIsNotifyDropOpen] = useState(false);
  const [isProfileDropOpen, setIsProfileDropOpen] = useState(false);

  const notifyDropRef = useRef<HTMLLIElement>(null);
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (
        ...
        !notifyDropRef.current.contains(event.target as Node)
      ) {
        setIsNotifyDropOpen(false);
      }
      if (
  ...
  const toggleNotifyDrop = () => {
    setIsNotifyDropOpen(!isNotifyDropOpen);
    setIsProfileDropOpen(false); // Close profile dropdown when opening notify
  };

  const toggleProfileDrop = () => {
    setIsProfileDropOpen(!isProfileDropOpen);
    setIsNotifyDropOpen(false); // Close notify dropdown when opening profile
  };
      ...
      <div className="container _custom_container">
        ...
        <div className="navbar-collapse" id="navbarSupportedContent">
          ...
          <ul className="navbar-nav mb-2 mb-lg-0 _header_nav_list ms-auto _mar_r8">
            ...
            <li className="nav-item _header_nav_item" ref={notifyDropRef}>
              <span
